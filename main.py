...
while True : 
    n1 = int(input("Please enter how big you want your board to be (q x q). Values must be between (3 - 9): "));
    if 3 <= n1 <= 9 : 
        break;
WIDTH = n1;
HEIGHT = n1;
...
for key in trackings : 
    if len(trackings[key]) == 0 : 
        continue;
    if len(trackings[key]) == 2 and carved_board[trackings[key][0][0]][trackings[key][0][1]] % carved_board[trackings[key][1][0]][trackings[key][1][1]] == 0 : 
        key_opp = "/";
        evaluations = [str(carved_board[trackings[key][0][0]][trackings[key][0][1]]) , "/" , str(carved_board[trackings[key][1][0]][trackings[key][1][1]])]
    elif len(trackings[key]) == 2 and carved_board[trackings[key][1][0]][trackings[key][1][1]] % carved_board[trackings[key][0][0]][trackings[key][0][1]] == 0 : 
        key_opp = "/";
        evaluations = [str(carved_board[trackings[key][1][0]][trackings[key][1][1]]) , "/" , str(carved_board[trackings[key][0][0]][trackings[key][0][1]])]
...
q = 50;
my_canvas = canvas.Canvas("ken_ken_board.pdf" , pagesize=((q * 2) + (WIDTH*q) , (q * 2) + (HEIGHT*q)));
my_canvas.setFont("Helvetica", 8)

my_answers = canvas.Canvas("ken_ken_answers.pdf" , pagesize=((q * 2) + (WIDTH*q) , (q * 2) + (HEIGHT*q)));
...
def draw_out_canvas() :  
    ...
    for i in range(len(splitted_board)) : 
        for j in range(len(splitted_board[i])) : 
            ...
            curr_i = q * (i + 1) - q;
            curr_j = q * (j + 1); - q;
            upper_bound , lower_bound , left_bound , right_bound = False , False , False , False;
            ...
            if upper_bound == True : 
                ...
                #print(curr_i + 25 , col);
                my_canvas.line(curr_j, tempf - curr_i + q , curr_j + q , tempf - curr_i + q)
                my_canvas.setLineWidth(1);
            else :  
                my_canvas.line(curr_j, tempf - curr_i + q , curr_j + q , tempf - curr_i + q)
            if lower_bound == True : 
            ...
            if right_bound == True : 
                my_canvas.setLineWidth(5);
                my_canvas.line(curr_j + q, tempf - curr_i , curr_j + q , tempf - curr_i + q)
                my_canvas.setLineWidth(1);
            else : 
                my_canvas.line(curr_j + q, tempf - curr_i, curr_j + q , tempf - curr_i + q)
    ...
    for i in range(len(splitted_board)) : 
        for j in range(len(splitted_board[i])) : 
            ...
            if right_bound == True : 
                ...
                my_canvas.line(curr_j + q, tempf - curr_i, curr_j + q , tempf - curr_i + q)
            my_canvas.drawString(curr_j + int(q/2) - 3 , tempf - curr_i + int(q/2) - 3 , str(carved_board[i][j]));
            ares_dones[col] = True;
draw_out_canvas();
